...
export class InMemoryBackstoryRepository implements BackstoryExtractor, CharacterBackstoryExtractor {
  private backstories: Map<BackstoryId, Backstory> = new Map<BackstoryId, Backstory>();

  constructor() {
    stories.map(s =>
      this.backstories.set(s.id, new Backstory(s.id, s.name, s.description, s.inventoryItems, s.abilities))
    );
  }
